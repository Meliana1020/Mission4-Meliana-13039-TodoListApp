document.getElementById("add-task").addEventListener("click", function () {
  ...
  if (startTime && endTime && endTime < startTime) {
    alert("Jam selesai tidak boleh lebih awal dari jam mulai, bre!");
    return;
  }
  ...
  dateCell.textContent = `${taskDate} (${timeRange})`;
...
function saveToLocalStorage(
  ...
  data.push({ taskText, taskPriority, taskDate, startTime, endTime });
...
document.getElementById("show-done").addEventListener("click", function () {
  ...
  doneTasks.style.display =
    doneTasks.style.display === "none" ? "block" : "none";
});
...
document.getElementById("show-overdue").addEventListener("click", function () {
  ...
  overdueTasks.style.display =
    overdueTasks.style.display === "none" ? "block" : "none";
});
...
function addTaskToTable(taskText, taskPriority, taskDate, isDone) {
  ...
  const actionCell = newRow.insertCell(3);
  const timeDisplay =
    startTime && endTime ? `${taskDate} (${startTime} - ${endTime})` : taskDate;

  taskCell.textContent = taskText;
  priorityCell.textContent = taskPriority;
  dateCell.textContent = taskDate;
  dateCell.textContent = timeDisplay;
